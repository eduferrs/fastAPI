...
async def get(id: UUID4, db_session: DatabaseDependency) -> AtletaOut:
    atleta: AtletaOut = (await db_session.execute(select(AtletaModel).where(AtletaModel.id == id))).scalars().first()
...
@router.patch(path="/{id}", summary="Editar atleta pelo id", status_code=status.HTTP_200_OK, response_model=AtletaOut)
